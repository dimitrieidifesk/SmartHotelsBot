    ...
    if message.text.isdigit():
        ...
        if price >= 0:
            if price - min_price > 0:
                ...
                bot.send_message(
                    chat_id, f"Выбранный диапазон цен {min_price} - {price} долларов $"
                )
                bot.send_message(
                    chat_id,
                    "Введите желаемое минимальное расстояние до центра города (целое число, километры):",
                )
            else:
                bot.send_message(
                    chat_id, f"Ошибка, введите число больше, чем минимальное - {min_price}."
                )
        else:
