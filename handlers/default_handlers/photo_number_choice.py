...
@bot.message_handler(
    func=lambda message: "choice_photo_number" in get_state(message.chat.id, "states")
)
@logger.catch
def choice_photo_number(message: Message) -> None:
    ...
    if message.text.isdigit():
        ...
        if number > 10 or number < 1:
            ...
            if command == "lowprice":
                request_hotels(chat_id, "PRICE_LOW_TO_HIGH")
            elif command == "highprice":
                request_hotels(chat_id, "PROPERTY_CLASS")
